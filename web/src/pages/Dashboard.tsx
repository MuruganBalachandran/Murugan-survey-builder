...
export const DashboardPage = () => {
  ...
  const { surveys, surveysTotal, isLoading } = useAppSelector(
    (state) => state.survey,
  );
  const { responses, isLoading: responsesLoading } = useAppSelector(
  ...
  const dashboardData = useMemo(() => {
    ...
    const answeredFields = typedResponses.reduce(
      (sum, response) =>
        sum +
        response.answers.filter((answer) => isAnswered(answer.value)).length,
      0,
    );
    const totalFields = typedResponses.reduce(
      (sum, response) =>
        sum + Math.max(response.questionCount, response.answers.length),
      0,
    );
    const completionRate =
      totalFields > 0 ? Math.round((answeredFields / totalFields) * 100) : 0;
    ...
    return {
      ...
      topSurveys: [...surveys]
        .sort(
          (first, second) =>
            (second.responseCount ?? 0) - (first.responseCount ?? 0),
        )
        ...
        {dashboardData.draftSurveys.length > 0 && (
          <section className="rounded-2xl border border-amber-200 bg-amber-50 p-6 shadow-sm">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-600">
                  These surveys are still drafts and need finishing before
                  sharing.
                </p>
              </div>
            </div>
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {dashboardData.draftSurveys
                .slice(0, DRAFT_SURVEYS_PREVIEW_LIMIT)
                .map((survey) => (
                  <div
                    key={survey.id}
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-gray-500">
                          ...
                          {(survey.responseCount ?? 0).toLocaleString()}{" "}
                          responses
                        </p>
                      </div>
                  ...
                  <div key={survey.id}>
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          navigate({ to: `/surveys/${survey.id}/responses` })
                        }
        ...
        <section className="grid gap-6 lg:grid-cols-1">
          <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                Latest responses across your surveys
              </p>
            </div>
